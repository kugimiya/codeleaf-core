...
import {
  DeepGet, DeepKeys, DeepKeysFiltered, UnpackedArray,
} from '../utils/types';
...
export default class Store<T extends Record<string, unknown>> {
  ...
  >(
    ...
    this.setInPath(path, (item) => (
      (item as ItemType[])
        .map((subItem) => (compareFn(subItem) ? { ...subItem, ...newValue } : subItem))
    ));
  }
   ...
   */
  setInPath<Keys extends DeepKeys<T>>(path: Keys, cb: (item: DeepGet<Keys, T>) => void): void {
